...
class pos_order(osv.osv):
    ...
    def create(self, cr, uid, values, context=None):
        """
        This function gets the unique name for PoS orders using sequence
        to avoid error by concurrent calling to this function by several
        PoS try five times with 50ms sleep time in between
        """
        from time import sleep
        sequence_object = self.pool.get('ir.sequence')
        for attempt in range(5):
            try:
                values['name'] = sequence_object.get(cr, uid, 'pos.order')
            except:
                sleep(0.05)
            else:
                break
            ...
            raise osv.except_osv(
                _('Error'),
                _('Unable to get a number for sequence!')
            )
